...
export const clinicView = {
  ...
  render(ctx) {
    ...
    let out = card(
      ...
      + '<p style="margin:8px 0 0"><small>Photo review, treatment plans checked against every '
      + 'rule before they are sprayed, the dose per 16 L knapsack and per tank, three pH readings '
      + 'turned into a lime route and kilograms, gate evidence, the three-day check after a spray '
      + '— and the adviser that reads your own records. The rules-based half works with no '
      + 'network.</small></p>',
      { tight: true },
    );
